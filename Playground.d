...
/++
    // We will be creating H20 (Water). So, for every Oxygen, there will be two Hydrogen.
    private void createAllParticles( double minimumInitialDistance, size_t totalNumberOfH, size_t totalNumberOfO )
    {
    	size_t pi = 0;

    	p = new Particle[ n ];

        auto remainingH = totalNumberOfH;
        auto remainingO = totalNumberOfO;

        while( (remainingH > 0) || (remainingO > 0) )
        {
            if( (remainingH > 0) && (remainingO > 0) )
            {
                auto decision = std.random.uniform!"[]"( 0, 1 );

                switch( decision )
                {
                    case 0:
                        p[ pi++ ] = createFarEnoughParticle( minimumInitialDistance, radiusH, +1.0 );
                        --remainingH;
                        break;

                    case 1:
                        p[ pi++ ] = createFarEnoughParticle( minimumInitialDistance, radiusO, -2.0 );
                        --remainingO;
                        break;
        ...
        /*
    	for(auto i=0; i < totalNumberOfH; ++i)
    	{
    		p[ pi++ ] = createFarEnoughParticle( minimumInitialDistance, radiusH, +1.0 );
    	}

    	for(auto i=0; i < totalNumberOfO; ++i)
    	{
    		p[ pi++ ] = createFarEnoughParticle( minimumInitialDistance, radiusO, -2.0 );
    	}
        */
    ...
    private Particle createFarEnoughParticle( double minimumInitialDistance, double r, double q )
    {
    	double delta = minimumInitialDistance * 10.0;
    	double delta_neg = -1*delta;

    	Particle particle = new Particle( 0, 0, 0, r, q );

    	while( true )
    	{
	    	for(size_t i=0; i < n; ++i)
	    	{
	    		Particle otherParticle = p[i];

	    		// no other particle to check whether the are too close. no movement was required due to previous ones either.
	    		// so, we can place the new particle to its current place.
	    		if( otherParticle is null ){
	    			return particle;
	    		}

	    		if( particle.tooClose( particle, otherParticle, minimumInitialDistance ) )
	    		{
	    			break;
	    		}
	    	}
	    	...
	    	// move position
	    	particle.moveX( std.random.uniform( delta_neg, delta ) );
	    	particle.moveY( std.random.uniform( delta_neg, delta ) );
	    	//particle.moveZ( std.random.uniform( delta_neg, delta ) );   // I do not want Z (depth) due to visulisation problem

	    	while( particle.y > (_boxTop - minimumInitialDistance) )
	    	{
		    	particle.moveY( std.random.uniform( delta_neg, 0 ) );
	    	}

	    	while( particle.y < (_boxBottom + minimumInitialDistance) )
	    	{
		    	particle.moveY( std.random.uniform( 0, delta ) );
	    	}
	    }
    }
++/
